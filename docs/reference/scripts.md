...
- `bun run dist:desktop:linux` — Builds Linux AppImage and `.tar.gz` artifacts into `./release`.
- `bun run dist:desktop:linux:tarball` — Builds only the portable Linux `.tar.gz` artifact.
